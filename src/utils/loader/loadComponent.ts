...
async function init(resolve: (arg: any) => void, reject: (arg: any) => void, scope: string, component: string) {
  try {
    // @ts-ignore
  ...
  return () =>
    new Promise((resolve, reject) => {
      ...
      if (externals.hasLoadedApp(app)) {
        ...
        const head = document.querySelector('head') || document.body.appendChild(document.createElement('head'));
